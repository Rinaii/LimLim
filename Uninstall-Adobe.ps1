...
$source = "\\Kode\main\Installs\Adobe Uninstall"
$destination = "\\$computer\c$\Laurel"
#Insert the file name "PC_Uninstall.txt" instead of test1.txt
$computers = Get-Content -path "\\Kode\main\Installs\Adobe Uninstall\PC_adobetest.txt"
write-host "Start copying the files from '\Kode\main\Installs\Adobe Uninstall\*' to all computers" -ForegroundColor Green
foreach ($computer in $computers){
copy-item -path "\\Kode\main\Installs\Adobe Uninstall\*" -destination "\\$computer\c$\laurel" -verbose
...
Remove-Item -Path C:\Windows\System32\Macromed\Flash* -Verbose
